...
def get_reason_text(reason_code: str) -> str:
    ...
    if reason_code == "tests":
        return "💎 PREMIUM ДОСТУП — ВСЕ ТЕСТЫ БЕЗ ОГРАНИЧЕНИЙ 💎

🚀 Открой полный доступ ко всей базе платных тестов всего за:
...
✅ Quiz-формат как на реальном экзамене
✅ Таймер и проверка результатов
✅ Новые тесты сразу после добавления
✅ Доступ 24/7 с телефона и ПК

🔥 Готовься без ограничений и прокачивай свои результаты каждый день!

📩 Чтобы приобрести Premium — напиши хочу премиум, админ свяжется с вами."
