...
class FsmStorageEntry(Base):
    """Backs PostgresStorage (app/services/fsm_storage.py) — FSM state/data for
    ongoing conversations, persisted so a bot restart never silently loses an
    in-progress admin flow (entering an amount, editing requisites, etc.) or a
    user's ticket-composition state. Used automatically whenever REDIS_URL isn't
    configured; see app/bot.py::create_dispatcher.
    """
