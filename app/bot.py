...
def create_dispatcher() -> Dispatcher:
    ...
    if settings.REDIS_URL:
        ...
        # No separate Redis service needed: conversation/admin-flow state is kept in the
        # same PostgreSQL database the bot already requires, so a restart (deploy, crash,
        # platform recycling the container) can never silently lose an in-progress flow
        # the way in-memory storage would.
        from app.db.session import async_session_maker

        storage = PostgresStorage(async_session_maker)
        logger.info(
            "REDIS_URL is not set — using PostgreSQL-backed FSM storage (durable across "
            "restarts). Spam-guard counters still reset on restart, which is harmless."
        )
    ...
    # Absolute last-resort diagnostic: catches any admin-chat message that nothing
    # above claimed at all (not even a reply), so a silent failure becomes visible
    # and logged instead of the bot saying nothing whatsoever.
    dp.include_router(admin_catch_all.router)
