...
from app.services.spam_guard import create_spam_guard
...
SENT_CONFIRMATION_TEXT = "Сообщение отправлено, ожидайте ответ."
SENT_CONFIRMATION_AUTO_DELETE_SECONDS = 5

# Keeps references to fire-and-forget auto-delete tasks so asyncio can't garbage-collect
# them mid-sleep (same pattern used for the admin-side confirmation).
...
@router.message(UserFlow.chatting, F.chat.type == "private")
async def relay_to_admin(message: Message, state: FSMContext, session: AsyncSession, redis: Redis | None) -> None:
    ...
    confirmation = await message.answer(SENT_CONFIRMATION_TEXT)
    task = asyncio.create_task(_delete_after_delay(confirmation, SENT_CONFIRMATION_AUTO_DELETE_SECONDS))
